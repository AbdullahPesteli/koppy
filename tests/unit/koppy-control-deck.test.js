...
test("live control deck exposes and clears Recent Copies without using preferences", () => {
    const dom = new JSDOM("<!doctype html><html><body></body></html>");
    ...
    const config = makeConfig(prefs);
    let stack = { enabled: false, count: 0, bytes: 0, ready: false, accepted: false, maxItems: 10, maxBytes: 150 * 1024 * 1024 };
    let listener;
    const deck = Deck.install({
        document: dom.window.document,
        window: dom.window,
        ...
        getStackState: () => stack,
        acceptRecentCopies() {
            stack = Object.assign({}, stack, { accepted: true });
            listener(stack);
            return stack;
        },
        clearStack() {
    ...
    deck.show();
    const root = dom.window.document.querySelector("koppy-control-deck").shadowRoot;
    assert.equal(root.querySelector(".recent-copies"), null);
    assert.equal(config.saves, 0);

    stack = Object.assign({}, stack, { count: 2, bytes: 5 * 1024 * 1024, ready: true });
    listener(stack);
    assert.equal(root.querySelector(".recent-copies").textContent, "Son 2");
    root.querySelector(".recent-copies").click();
    assert.equal(stack.accepted, true);
