...
PNG olarak panoya kopyalar. PDF ve PDF-uyumlu kaydedilmiş Illustrator (`.ai`) dosyaları için ilk sayfa
yerelde PNG'ye render edilir. Ayar paneli Koppy'ye özel, aranabilir ve responsive'tir.
...
imlecin yanındaki `▣ 1` rozetine uçar. Sonraki normal `⌘C` kopyaları sayıyı artırır; rozet sayfada imleci
...
Koppy dosyanın uzantısına veya sunucunun MIME başlığına güvenmez; PNG, JPEG, WebP, GIF, BMP, ICO, AVIF ve
tarayıcının çözebildiği SVG girdilerini içerik imzasından tanır ve standart `image/png` çıktısı üretir.
PDF ve PDF-uyumlu AI için ilk sayfa, indirilen dosyadan yerelde render edilir; kaynak URL'ye ikinci bir
istek veya URL-metni fallback'i yoktur. Eski PostScript tabanlı AI/EPS, PSD/RAW gibi tarayıcının çözmediği
dosya aileleri evrensel bir pano görseli değildir: Koppy bunları yanlış/boş bir görsel diye kopyalamaz ve
nedenini açıkça bildirir. Bunları kapsamak, ayrı izinle kurulacak yerel dönüştürücü köprüsünün işidir.
PDF/AI render'ı ham dosya boyutu, sayfa çözünürlüğü ve üretilen PNG için mevcut 80 MB / 40 megapiksel
sınırlarını korur; yalnız ilk sayfayı işler.
